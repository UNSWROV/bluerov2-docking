...
class VelocityLoop:
    ...
    def step(self, v_ref: np.ndarray, v_meas: np.ndarray, dt: float) -> np.ndarray:
        ...
        long saturation does not wind it up.
        """
        ...
        raw = p.kp * err + i_term
        out = np.clip(raw, -p.effort_max, p.effort_max)
        saturated = (np.abs(raw) > p.effort_max) & (np.sign(raw) == np.sign(err))
...
def effort_from_setpoint(
    ...
    """One velocity-loop step, or the setpoint passed through as effort when no
    navigation velocity is available (the open-loop law, so the vehicle keeps
    regulating rather than stopping)."""
    if v_meas is None:
        return np.asarray(v_ref, dtype=float)
    return loop.step(v_ref, v_meas, dt)
